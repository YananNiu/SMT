...
class vit_model_img_ts_referencetoken(nn.Module):
    """ Vision Transformer with LayerScale (https://arxiv.org/abs/2103.17239) support
    taken from https://github.com/rwightman/pytorch-image-models/blob/master/timm/models/vision_transformer.py
    with slight modifications
    """
    def __init__(self, img_size=224,  patch_size=16, ts_shape = (1,144), in_chans=3, num_classes=1, embed_dim=768, depth=12,
                 ...
                 block_layers = Block,
                 Patch_layer=PatchEmbed,act_layer=nn.GELU,
                 Attention_block = Attention, Mlp_block=Mlp,init_scale=1e-4,**kwargs):
        super().__init__()
        ...
        # cls_token: a special learnable embedding for the whole image, prepended to the sequence.
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.reference_token = nn.Parameter(torch.ones(1, 1, embed_dim))
    ...
    def forward_features(self, x, ts,reference):
        assert x.shape[0] == ts.shape[0]
        B = x.shape[0] 
        cls_tokens = self.cls_token.expand(B, -1, -1)
        # add reference token
        reference_tokens = self.reference_token.expand(B, -1, -1)
        reference = reference.reshape(B,1,-1).expand(B,1,self.embed_dim)
        reference_tokens = reference_tokens*reference

        x = self.patch_embed(x)
        x = x + self.img_pos_embed + self.token_type_embeddings(torch.ones(B, x.shape[1], dtype=torch.long,device=x.device))
        x = torch.cat((cls_tokens,reference_tokens,x), dim=1)
    ...
    def forward(self, x,ts,reference):
        x = self.forward_features(x,ts,reference)
